 ...
 */
export class EventStatusWordFlag extends CosemInterfaceClass {
    // Event Status Word bytes
    private eswBytes: Map<number, number> = new Map();
    
    // Constants for bit positions
    private static readonly ESW_BITS = {
    ...
    constructor() {
        super(ObisCode.fromString('0.0.96.11.0.255'), 1);
        this.initializeESWBytes();
        this.initializeAttributes();
    }

    private initializeESWBytes(): void {
     ...
     * Set event status
     */
    public setEventStatus(eventName: keyof typeof EventStatusWordFlag.ESW_BITS, status: boolean): void {
     ...
     * Get event status
     */
    public getEventStatus(eventName: keyof typeof EventStatusWordFlag.ESW_BITS): boolean {
        const eventConfig = EventStatusWordFlag.ESW_BITS[eventName];
        if (!eventConfig) {
            throw new Error(`Unknown event: ${eventName}`);
        }
     ...
     * Get ESW byte value
     */
    public getESWByte(byteNumber: number): number {
     ...
     * Set ESW byte value
     */
    public setESWByte(byteNumber: number, value: number): void {
        if (byteNumber < 0 || byteNumber > 15) {
            throw new Error('Invalid ESW byte number. Must be between 0 and 15.');
        }
