...
describe('ObisCode', () => {
    describe('constructor', () => {
        it('should create OBIS code with valid values', () => {
            const obis = new ObisCode(1, 0, 1, 8, 0, 255);
            expect(obis.toString()).toBe('1.0.1.8.0.255');
        });

        it('should throw error for invalid values', () => {
            expect(() => new ObisCode(-1, 0, 0, 0, 0, 0)).toThrow();
            expect(() => new ObisCode(256, 0, 0, 0, 0, 0)).toThrow();
        });
    });

    describe('parsing', () => {
        it('should parse from string', () => {
            const obis = ObisCode.fromString('1.0.1.8.0.255');
        ...
        it('should parse from buffer', () => {
            ...
            const obis = ObisCode.fromBuffer(buffer);
            expect(obis.toString()).toBe('1.0.1.8.0.255');
        });

        it('should throw error for invalid string format', () => {
            expect(() => ObisCode.fromString('1.0.1')).toThrow();
            expect(() => ObisCode.fromString('1.0.1.8.0.256')).toThrow();
            expect(() => ObisCode.fromString('invalid')).toThrow();
        });

        it('should throw error for invalid buffer', () => {
            expect(() => ObisCode.fromBuffer(Buffer.from([1, 0, 1]))).toThrow();
        });
    });

    describe('comparison', () => {
        it('should compare OBIS codes', () => {
            const obis1 = new ObisCode(1, 0, 1, 8, 0, 255);
            const obis2 = new ObisCode(1, 0, 1, 8, 0, 255);
            const obis3 = new ObisCode(2, 0, 1, 8, 0, 255);
        ...
        it('should match wildcard values', () => {
            const obis1 = new ObisCode(1, 0, 1, 8, 0, 255);
            const pattern = new ObisCode(1, 255, 1, 8, 255, 255); // B and E are wildcards
    ...
    describe('validation', () => {
        it('should validate individual values', () => {
            expect(() => new ObisCode(256, 0, 0, 0, 0, 0)).toThrow();
            expect(() => new ObisCode(0, 256, 0, 0, 0, 0)).toThrow();
            expect(() => new ObisCode(0, 0, 256, 0, 0, 0)).toThrow();
            expect(() => new ObisCode(0, 0, 0, 256, 0, 0)).toThrow();
            expect(() => new ObisCode(0, 0, 0, 0, 256, 0)).toThrow();
            expect(() => new ObisCode(0, 0, 0, 0, 0, 256)).toThrow();
        });

        it('should validate array values', () => {
            expect(() => ObisCode.fromArray([-1, 0, 0, 0, 0, 0])).toThrow();
            expect(() => ObisCode.fromArray([0, 0, 0, 0, 0])).toThrow(); // Too short
            expect(() => ObisCode.fromArray([0, 0, 0, 0, 0, 0, 0])).toThrow(); // Too long
        });
    });

    describe('utility methods', () => {
        it('should check if value is wildcard', () => {
            const obis = new ObisCode(1, 255, 1, 8, 255, 255);
