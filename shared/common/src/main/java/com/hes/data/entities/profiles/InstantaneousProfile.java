...
@Setter
@Entity
@Table(name = "instantaneous_profiles")
public class InstantaneousProfile extends BaseEntity {
    @Id
    ...
    private Long capturePeriod = 0L; // Set to 0 for on-demand immediate data capture
    ...
    private Long entriesInUse = 1L; // Current number of entries in buffer: 1

    @Column(name = "profile_entries")
    private Long profileEntries = 1L; // Should be limited to 1

    @Column(name = "obis_code", nullable = false)
    private String obisCode = "1.0.94.91.0.255";

    @OneToMany(mappedBy = "profile", cascade = CascadeType.ALL, orphanRemoval = true)
    private List<InstantaneousCapture> captures;
} 