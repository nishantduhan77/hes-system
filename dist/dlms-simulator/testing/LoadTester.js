...
class LoadTester extends events_1.EventEmitter {
     ...
     */
    async startWorker(testId, config) {
        while (this.activeTests.has(testId)) {
            for (const operation of config.operations) {
                ...
                try {
                    ...
                    this.logger.logError('LoadTester', error);
                }
            }
        }
    }
     ...
     * Update test metrics
     */
    updateMetrics(testId, success, responseTime) {
        ...
        metrics.averageResponseTime = ((metrics.averageResponseTime * (metrics.totalRequests - 1) + responseTime) /
            metrics.totalRequests);
    ...
    completeTest(testId) {
        ...
        const result = {
            testId,
            testType: TestFrameworkManager_1.TestType.LOAD,
     ...
     */
    async simulateOperation(minTime, maxTime) {
